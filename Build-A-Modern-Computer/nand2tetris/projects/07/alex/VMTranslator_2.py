...
class VMTranslator:
    ...
    def parse_stack_ops(self, cmd, segment, index):
        ...
        elif cmd == POP:                        # pop local 0
            self.set_address_to_A(segment, idx) # A = resolved address --> find the pop position 
            self.asm += ["D=A"]                 # D = A
            self.asm += ["@R13", "M=D"]         # *R13 = D. R13 stores address of segment index
            self.pop_stack_to_D()               # D = stack.pop()
            self.asm += ["@R13", "A=M", "M=D"]  # RAM[*R13] = RAM[address] = D

    def set_address_to_A(self, segment, index):
        # A = resolved address
        if segment in PREDEFINED_SEGMENTS:      # pop local 0
            address_sym = PREDEFINED_SEGMENTS[segment] 
            self.asm += [f"@{address_sym}", "D=M", f"@{index}", "A=D+A"] # find the address of local 0
